...
def texto_a_silabas(texto,a):
    ...
    lista_silabas = [texto_sin_espacios[i:i+a] for i in range(0, len(texto_sin_espacios), a)]
...
def RSA_cifrar(texto_claro,clave_publica):
  ...
  cipher_blocks = [str(cifrar(base_alphabet_to_26(silaba),clave_publica)) for silaba in texto_claro]
...
def rsa_encrypt(clear_text: str, key: str):
    ...
    cipher_blocks = [str(cifrar(base_alphabet_to_26(silaba),key)) for silaba in clear_text]
